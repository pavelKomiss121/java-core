...
class RockPaperScissorsTest {
  ...
  void generateComputerChoiceReturnsValidOption() {
    ...
    assertThat(choice.toLowerCase()).isIn(ROCK.toLowerCase(), SCISSORS.toLowerCase(),
        PAPER.toLowerCase()); // В этой строке ошибка!
  }
  ...
  @CsvSource({ // playerChoice, computerChoice, expectedResult
      ...
      // В этой строке ошибка! Бумага бьет камень → Победа компьютера
      "Бумага, Ножницы, Победа компьютера"
      // В этой строке ошибка! Ножницы бьют бумагу → Победа компьютера
  })
  @DisplayName("Различные комбинации выборов для determineWinner")
  void testVariousChoiceCombinationsDetermineWinner(String playerChoice, String computerChoice,
