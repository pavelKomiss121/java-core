...
class NumberGuessingGameTest {
  ...
  void testPlayRoundGuessCorrectly() {
    ...
    NumberGuessingGame game = new NumberGuessingGame(new TestableRandom(42));
  ...
  void testStatisticsUpdated() {
    ...
    assertThat(output).contains(
        "Максимальное количество попыток: 5"); // Максимум (из второго раунда)
  ...
  static class TestableNumberGuessingGame extends NumberGuessingGame {
    ...
    TestableNumberGuessingGame(int fixedNumber) {
      super(new TestableRandom(fixedNumber)); // вызываем нужный конструктор родителя
